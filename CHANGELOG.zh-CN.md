...
- [Web] 新增公共个人主页 `/profile.html`,排行榜与管理端的用户标签全部直达:人物名片与人设标签、bento 指标组(最佳单日、缓存杠杆、出勤饱和度等)、多周期对比 Scoreboard、月度排名战绩、全史活动热力图、多维明细(趋势 / 节律 / 构成 / 模型 / 来源)与可筛选的数据日志工作台;旧详情弹层退役,卡片与标签直达主页。
- [Web] 前台「个人」Tab 由下拉切换器改为纯链接,同时消除了与其他页面浮层控件的层级冲突。

### 修复

- [Desktop] 修复 Codex 归档会话导致历史用量静默缩水的问题。Codex 会定期把会话文件从 `sessions/YYYY/MM/DD/` 挪入平铺的 `archived_sessions/`;此前采集器把完整路径算进源指纹,这种移动会使整个缓存失效并触发全量重算,而重算结果依赖文件处理顺序,偏低的总量会覆盖服务器历史(实测一天内 8 月总量从 70 亿掉到 68.3 亿)。现在指纹改由内嵌会话 UUID 的文件名推导,两目录同名副本自动去重,解析顺序成为文件集合的确定函数——已在 1,076 个真实会话文件上验证:归档前后两种目录布局产出逐文件一致的总量,并与 ccusage 对 158 个完整日逐 token 完全相等。
- [Desktop] 扫描不再在 1,000 个文件处静默截断 Codex 与 Claude Code 来源:重度用户的 Codex 会话文件已超过 1,000 个,旧上限会不可见地丢弃超出部分并使总量依赖目录读取顺序;上限现提升至 10,000。
- [Web] 修复 `data-i18n` 重翻译覆盖动态渲染名字的问题;个人主页原硬编码文案(人设标签、特性标签、巅峰月度、工作日/周末)全部接入双语,并清理 52 个孤儿翻译键。

### 变更

- [Web] 排行榜与月度排名按 businessDay 缓存并在写入路径自动失效,trend 接口新增 `fields=totals` 轻量模式;全联盟数据下个人主页约 200ms、排行榜约 5ms。

## [0.7.14] - 2026-08-28

### 新增

- [Web] 公榜新增来源维度：首页来源 Top3 模块、榜单按来源筛选、管理端来源统计标签页，由新增的服务端来源统计与来源维度排名接口支撑；下载页来源清单同步对齐来源维度。

### 修复

- [Desktop] 修复 Cursor 用量断流：cursor.com 的 Vercel 安全检查点开始按 TLS 指纹拦截 rustls（403 挑战页），采集器已切换到系统原生 TLS 栈（macOS SecureTransport / Windows schannel / Linux OpenSSL），Windows 与 macOS 真机均验证通过。旧客户端重连账号无法恢复，必须升级到本版本。
- [Desktop] Cursor 鉴权失败不再静默：30x 登录重定向与 401/403 统一按鉴权错误处理，触发 token 刷新，刷新无效的账号可见地翻为"需要重连"，不再停留 active 却零行；诊断导出新增上次扫描的 partial 标记、失败来源列表与逐来源错误。
- [Desktop] 上次有数据、本次零行且无报错的来源保留原有本地数据，不再被静默清空；全量对账不再 prune 上次扫描失败的来源的服务端数据——上游故障不会再破坏本地或服务器历史。
- [Desktop] Cursor 账号处理加固：会话 cookie 改用 token 内嵌的规范 WorkOS 用户 id 构造；来源页显示账号最后活跃日（从原始事件流推导，近期全是 Pro included 零 token 用量也可见）；历史缺失 email 的账号机会性修复；账号哈希不再作为界面标签展示。
- [Desktop] WorkBuddy 工作目录归因在 pid 目录复用场景下保持稳定：归因快照按扫描刷新，不再固化过期 cwd，重扫旧 trace 仍落到正确的工作目录。

### 变更

- Linux 构建环境脚本新增安装 `libssl-dev`（native-TLS 采集器的构建依赖），自建 Linux 构建机升级前需重跑初始化脚本。

## [0.7.13] - 2026-08-24

### 新增

- [Desktop] 新增 Kimi 本地用量来源：扫描 Kimi 桌面客户端的本地 Agent 会话（daimon 内核 `wire.jsonl` 日志），按次统计 token（输入、输出、缓存读写）并归属到工作目录，同时计入后台标题生成用量；本地 Kimi 用量校验已并入 `npm run verify:ccusage`。
- [Web] 预估费用旁新增单价指标（每 100M token 价格），覆盖榜单前三奖牌卡、占比行、列表视图与管理端用量排名。

### 修复

- 服务端小时批次在大小写变体合并进同一桶后，不再把合并后的行数当作接收条目数返回，此前这会让客户端多走一轮重试队列才能确认上传。
- 服务端同一小时桶内大小写冲突的模型名（如 GLM-5.3 与 glm-5.3）现在会全部求和，不再静默覆盖到只剩最后一个变体；空转上传在确认前会核对已存储行。
- 本地扫描不再对"既被自动发现又被手动配置"的 Kimi 来源目录重复计数（根目录先规范化再去重），已在真实 Windows 安装上验证。
- Windows 上运行测试套件不再读写真实的 `~/.ai-token-league` 数据目录（测试改用 `ATL_HOME` 沙箱，Windows 会忽略 `HOME`）。

### 变更

- 发布与构建 CI 升级到 Node 24 运行时，并新增覆盖 Linux、Windows、macOS 的跨平台构建验证工作流。
...
- E2E 分享卡片保存断言不再与弹窗自动关闭产生竞态：改为等待自动关闭本身（取消与失败路径不关闭弹窗），随后断言持久化的保存状态。
...
- [Desktop] 新增 ZCode 本地数据源：扫描本机 ZCode 请求级用量库，采用缓存感知的 token 口径、重试去重，并支持按工作目录归属。

### 修复

- [Web] 管理后台自定义日期范围在任一日期输入变化时立即生效，不再被快捷范围选项覆盖待选日期。
...
- [Web] 公开排行榜的奖牌、Meter 和列表视图新增贡献占比展示。
- [Web] Analytics 新增全社区参与者排名，并在公开模式下保持匿名安全展示。

### 变更

- [Web] 统一刷新公开下载页、排行榜、Analytics 与 Admin 的响应式视觉体系，并补强加载反馈。
- 云端用量同步改为基于已签名的设备级快照和增量 bucket 更新，同时保持历史客户端安全兼容。

### 修复

- [Desktop] 修复工作目录别名保存后刷新或重启未持久化的问题。
- [Web] 修复 Admin 中所选用量记录不在当前已加载列表时无法打开详情的问题。
...
- [Desktop] 新增分享卡设置面板，支持本地预览、竖版导出，以及上周 / 上月等快捷分享区间。
- [Web] 公开下载页趋势卡新增参与者徽标，并将图表调整为滚动最近 30 天视图。

### 变更

- [Desktop] 优化分享卡自适应布局、竖版 mini-meter 样式和横竖切换动效，使导出画面更稳定、更易读。
- [Web] 重构 admin usage 操作区，拆分排名视图和聚合视图，并为大列表补充分页能力。

### 修复

- [Desktop] 修复分享卡尺寸与选择器联动问题，确保竖版模式、自动高度和预览交互与自动化验证保持一致。
- [Web] 修复 board/admin analytics 返回的参与者身份标签稳定性，避免趋势图和管理视图出现名称不一致。

## [0.7.5] - 2026-05-31

### 新增

- [Desktop] 新增概览分享卡，可将近期用量生成为带品牌信息的 PNG 图片用于分享。
- [Desktop] 新增桌面端侧边栏品牌 Logo 展示，并在分享卡输出中带出品牌标识。
- 新增后端与存储层的参与者品牌 / Logo 元数据支持，用于分享卡渲染。

### 修复

- [Desktop] 修复更新包下载完成后重启 / 安装更新按钮不可点击的问题。
- [Desktop] 调整侧边栏布局，避免重启更新按钮与品牌 Logo 相互遮挡。
...
- [Desktop] Cursor dashboard 用量默认扫描当前自然月和上一个自然月。

### 修复

- [Desktop] 修复 Cursor dashboard 时间戳解析，支持数字字符串、秒级时间戳和 ISO 时间，避免用量落入伪造的当天 0 点 bucket。
...
- [Web] 新增 admin 设备级云端数据重置，可清理单个设备而不删除参与者或其他设备数据。
- 新增本地质量门禁覆盖：renderer 单测、mock E2E 流程、性能检查、sidecar 集成、scan 集成和 shared 模块行为。

### 变更

- [Desktop] 改进后台 scan、refresh、sync 处理，提升大本地数据量下 reset、启动、区间切换和同步状态刷新的响应稳定性。
- [Desktop] 优化同步状态判定，使服务器切换、上传队列、部分同步和刷新错误能在桌面端给出更清晰的状态。
- [Desktop] 注册时现在会上报客户端 LAN IP / 网络信息，便于 admin 诊断。
- [Desktop] 更新客户端发布元数据、安装包版本引用和打包 preset resource 处理到 `0.7.1`。

### 修复

- [Desktop] 检查更新前会刷新云端健康状态，避免 updater 使用过期连接状态。
- [Desktop, Web] 兼容旧 Ed25519 PEM 签名，保证客户端签名和服务端验签可继续识别旧格式。
- [Web] 下载页现在会在单个平台安装包 metadata 不完整时继续渲染其他可用安装包信息。
