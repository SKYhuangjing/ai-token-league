...
export function normalizeSilentUpdateMode(value) {
  return SILENT_UPDATE_MODES.includes(value) ? value : DEFAULT_SILENT_UPDATE_MODE;
}
...
export function loadConfig() {
  ...
  return withFixedRuntimeConfig(JSON.parse(fs.readFileSync(CONFIG_PATH, "utf8")));
}

export function saveConfig(config) {
  ensureAppDir();
  fs.writeFileSync(CONFIG_PATH, `${JSON.stringify(withFixedRuntimeConfig(config), null, 2)}\n`);
}
...
function withFixedRuntimeConfig(config = {}) {
  return {
    ...
    silentUpdateMode: DEFAULT_SILENT_UPDATE_MODE
  };
}
...
export function importIdentity(identity, current = {}, { persist = true } = {}) {
  const config = {
    ...
    hideDockIcon: current.hideDockIcon ?? false,
    desktopAutoInitialized: false,
...
export function exportConfig(config) {
  const { syncStatus, apiConnection, lastSyncAt, lastSyncStatus, lastSyncApiBaseUrl, lastSyncError, updatedAt, createdAt, importedAt, autoRefreshEnabled, silentUpdateMode, ...rest } = config;
...
export function importConfig(imported) {
  ...
  const config = {
    ...
    hideDockIcon: imported.hideDockIcon ?? false,
    desktopAutoInitialized: false,
...
export function updateConfig(input = {}, current = loadConfig(), { persist = true } = {}) {
  ...
  const config = {
    ...
    hideDockIcon: input.hideDockIcon ?? current.hideDockIcon ?? false,
