...
// Chart repaint from held state — reused after a no-reload theme switch.
function renderHomeCharts() {
  if (!state.analyticsData) return;
  homeRenderTrendChart();
  homeRenderBarChart("#home-model-chart", state.analyticsData.models, { collapseAfter: 4 });
  homeRenderDonut();
  homeRenderHeatmap();
...
window.addEventListener("atl:themechange", () => {
  renderHomeCharts();
  if (state.leaderboardData) renderLeaderboardPreview(state.leaderboardData.items || []);
  renderGallery();
});
